...
struct native EventListener
{
	...
	var EventObjectRef					CallbackData;
	...
	structcpptext
	{
		...
		EventListenerReturn ProcessEventDelegate( FEventObjectRef& EventData, FEventObjectRef& EventSource, const UXComGameState* GameState, UObject* CallbackData );
		...
		friend FArchive& operator<<(FArchive& Ar, FEventListener*& EventListener)
		{
			...
			if( Ar.LicenseeVer() >= FXS_VER_EVENTLISTENER_CALLBACKDATA)
			{
				Ar << EventListener->CallbackData;
			}
...
var private{private} pointer CriticalSection;
...
native function UnRegisterFromEvent( const ref Object SourceObj, 
									Name EventID, 
									optional EventListenerDeferral Deferral = ELD_MAX, 
...
native function string EventManagerDebugString( optional name EventID='', optional string DelegateObjectClassType="", optional int SourceObjectFilter=-1, optional int PreFilterObject=-1, optional bool bShowPendingEvents); // -1 means any.
